...
You should write apps using vanilla, idiomatic TypeScript. The main exception to this idiomatic rule is to use `neverthrow` style Result types for handling success and failure. This library should be used instead of typical try/catch, whenever possible. That includes using the `ProblemDetails` and `StandardError` helpers from `forge-lib-codegen`.
...
Code linting rules are also enforced by `biome`. Use `npm run lint` to check for code style errors.
The testing framework is `vitest`. Use `npm run test` to check if tests are passing. Make sure to write tests when writing new code. Target 80% code coverage, focusing on the behaviors of the Forge app, not integration with the underlying runtime.

# Imports & Libraries

Whenever possible, you rely on the `forge-lib-codegen` library. This library has already been installed with `npm link`. Do not try to download it using `npm install`.
You may import additional packages from reputable npm libraries when needed.
...
When making API requests via the @forge/api library,
prefer the `getAuthForEvent` function in `forge-lib-codegen`.
...
The Forge Manifest declares Input parameters and their types.
Meanwhile, your function needs to repeat the same parameters & types.
Use the `actiontypes` script to generate the type(s) from the manifest.

1. Write the Forge manifest section for [Rovo Actions](https://developer.atlassian.com/platform/forge/manifest-reference/modules/rovo-action/). For example, you could copy/paste from [this manifest example](https://developer.atlassian.com/platform/forge/manifest-reference/modules/rovo-action/#example-using-a-hosted-forge-function-).
2. Run `npm run types`. Find newly generated `src/actionpayload.ts`.
3. Use those types to express the interface for the `payload` parameter in your implementation with the generated Payload type.
